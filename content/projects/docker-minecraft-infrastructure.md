...
> [!NOTE]
> 💡 **Minecraft サーバー構築における「手動セットアップの面倒くささ」とは？**  
> 通常、Minecraft のマルチプレイサーバーを自分で構築・運用しようとすると、非常に多くの手動作業と複雑な環境問題に直面します。  
> 例えば、Minecraft のバージョンごとに**必要な Java のバージョン（Java 8, 16, 17, 21 など）が異なり**、パソコン内に複数の Java が混在して動かなくなるトラブルが頻発します。さらに、設定ファイル（`server.properties` や EULA 規約の同意）の編集、ポート開放、複数ワールドの接続設定、定期バックアップなど、手動でのセットアップ作業は初心者にとって大きな障壁となります。  
> 本プロジェクトは、これらの複雑な手順を **Docker（コンテナ技術）でテンプレート化・固定化** し、誰でもコマンド一発（`docker compose up -d`）で環境構築から自動バックアップ、安全な複数サーバー間ネットワークまでを再現性高く起動できる仕組みを目指して設計しました。
